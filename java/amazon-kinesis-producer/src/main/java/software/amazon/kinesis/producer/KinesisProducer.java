...
public class KinesisProducer implements IKinesisProducer {
    ...
    private static final long DAEMON_MESSAGE_SILENCE_THRESHOLD_MS = 60_000;
    ...
    private void startDaemonHealthCheck() {
        if (config.getEnableDaemonHealthCheck()) {
            ...
            healthCheckExecutor.scheduleAtFixedRate(
                    ...
                    0,
                    // 20s intervals. this needs to be smaller than DAEMON_MESSAGE_SILENCE_THRESHOLD_MS
                    // so we get a few pings through before checking child status
                    DAEMON_MESSAGE_SILENCE_THRESHOLD_MS / 4,
    ...
    private void performHealthCheck() {
        ...
        long silenceThresholdMs = DAEMON_MESSAGE_SILENCE_THRESHOLD_MS + backoffTimeMs;
        if (timeSinceLastMessageMs <= silenceThresholdMs) {
            log.error("Daemon is healthy. Last message received was {}ms ago and silence threshold is {}ms",
                    timeSinceLastMessageMs, silenceThresholdMs);
            consecutiveRestartAttempts.set(0);
            return;
        }

        // circuit breaker. it prevents restarts if there is too many consecutive restarts
        if (consecutiveRestartAttempts.get() >= DAEMON_RESTART_MAX_ATTEMPTS) {
            log.error("Daemon silent for {}ms but max restarts reached ({}). Skipping restart.",
                    timeSinceLastMessageMs, DAEMON_RESTART_MAX_ATTEMPTS);
            return;
        }

        log.error("Daemon silent for {}ms and silence threshold is {}ms - restarting daemon (attempt {})",
                timeSinceLastMessageMs, silenceThresholdMs, consecutiveRestartAttempts.get() + 1);
